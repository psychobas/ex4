...
def removeFileOrFolder(f):
    if os.path.isfile(f):
        print(f'  - Removing file {f}')
        os.remove(f)
    if os.path.isdir(f):
        print(f'  - Removing dir {f}')
        shutil.rmtree(f)
...
if __name__ == '__main__':
    ...
    if pdfIndex > maxIndex or pdfIndex < 0:
        ...
        try:
            ...
            pdfpyfiles = set(
                glob.glob(exname + '/**/*.pdf', recursive=True) + glob.glob(exname + '/**/*.py', recursive=True))
            allfiles = set(glob.glob(exname + '/**/*.*', recursive=True))
            for f in allfiles.difference(pdfpyfiles):
                removeFileOrFolder(f)
            hiddenfilesorPaths = glob.glob(exname + '/**/.*') + glob.glob(exname + '/**/__*__')
